...
def call_agent(agent_info, user_prompt, max_tokens=3000, model="gpt-4o-mini"):
    ...
    默认使用 model="gpt-4o-mini"，但可在调用时自定义为更高级模型，如"gpt-o1-mini"。
    """
...
"""
    ...
    try:
        # 注意这里我们使用 openai.chat.completions.create，而不是 openai.ChatCompletion.create
        response = openai.chat.completions.create(
            model=model,  # 动态决定使用哪个模型
            messages=messages,
            temperature=0.7,
...
agents = {
    ...
    "deep_improvement_agent": {
        "role": "资深算法架构师与高级专利撰写顾问",
        "goal": "对已有专利草稿进行深度提升，并确保可专利性和技术可行性",
        "backstory": "拥有丰富的AI算法研发经验与专利撰写经历，对电池和车辆数据分析有深入研究。"
    }
}
...
tasks = {
    ...
    "customization": (
        ...
        "3. 整体输出应超过2000个字符，内容详尽、专业。\n",
        8000
    ),
    "deep_improvement": (
        "### Step5：深度改进\n\n"
        "**任务背景：**\n"
        "在前面 4 个步骤的基础上，进一步深挖专利中的技术细节、数学原理、深度学习算法流程、对比现有技术等，"
        "并对最终的专利文档进行精修和完善。\n\n"
        "**任务指令：**\n"
        "请你以**高级专利撰写顾问**与**资深算法架构师**的身份，从以下角度进行深度改进：\n"
        "1. 逐点审视前几步内容的可行性、逻辑性、创新性；\n"
        "2. 在算法和数学公式层面，给出更多技术细节（如模型结构、训练损失、数据预处理、阈值设定等）；\n"
        "3. 对标已有技术或竞品，对比分析其不足，突出发明的新颖性；\n"
        "4. 完善专利文档的结构，如分段细化：背景技术、发明内容、具体实施方式、附图说明、权利要求等。\n\n"
        "请结合之前四步的所有输出，再次整理出一份更丰富、更深入、更具技术含量的专利草稿。\n\n"
        "**要求：**\n"
        "1. 写出不少于 4000 字的内容；\n"
        "2. 数学公式请使用可在 Word 直接粘贴的形式；\n"
        "3. 逻辑严谨，段落清晰，务必体现技术深度与可实施性。\n",
        16000
    )
}
...
def build_prompt_for_step(step_name, tasks, results):
    ...
    elif step_name == "deep_improvement":
        # Step5 需要引用 Step1, Step2, Step3, Step4 的结果
        step1_output = results.get("initial_idea", "")
        step2_output = results.get("innovation", "")
        step3_output = results.get("qa", "")
        step4_output = results.get("customization", "")
        prompt = (
            base_prompt
            + "\n\n---\n\n"
            + "请参考之前四个步骤的全部内容：\n\n"
...
for task_name in ["initial_idea", "innovation", "qa", "customization", "deep_improvement"]:
    agent_key_map = {
        ...
        "deep_improvement": "deep_improvement_agent"
    }
    agent_key = agent_key_map[task_name]
    ...
    result = call_agent(
        ...
        model=model_to_use   # 在这里传入最终决定的模型
    )
